...
module.exports = exports = _.assign(RestApi(Users), {
	//lean userId
	queryLeanUser: function(req, res, next, id) {
	...
	//param with userId
	queryUser: function(req, res, next, id) {
		Utilities.validateObjectId(id, function(isValid) {
			if (!isValid) {
				return res.status(404).jsonp(Utilities.response(false, {}, 'Invalid user id', 404));
			} else {
	...
	inactiveUserById: function(req, res) {
		var user = req.userData;
		// user.update
		res.json({});
	},
